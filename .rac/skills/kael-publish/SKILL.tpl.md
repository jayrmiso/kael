...
- Ensure `.kael/worktrees/` is present in the local Git exclude.
- Refuse to publish if `.kael/worktrees/*` is already tracked. Worktrees are
  local runtime state and must not be hidden inside the handoff commit.
...
4. Commit any dirty `.kael/handoff.md`, verify no `.kael/worktrees/*` path is
   tracked, push the current branch, and open the PR with the installed wrapper:
...
- If `.kael/worktrees/*` is tracked, stop and report that it must be removed
  from the branch/index. Do not stage or commit it.
